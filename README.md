...
- 슬라이스
  ...
  - 선언방식
    ...
    - make함수(자료형, 길이, 용량[생략시, 길이])
      - ex) `slice5 []int = make([]int, 5, 10)` // int타입, 길이5, 용량10
      - ex) `slice8 := make([]int, 5)` // int타입, 길이5, 용량5
      - 만약 용량을 처음 설정보다 초과하면 재할당이 일어나 성능의 저하가 일어나며, 너무 적게쓰면 메모리의 낭비가 이루어짐. 길이와 용량을 엔지니어 입장에서 성능을 고려해서 디테일하게 사용할 수 있게 제공해준다.   
