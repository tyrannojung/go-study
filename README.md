...
- closure
  - 함수가 선언되는 순간. 함수가 실행 될 때 외부 변수에 접근하여 `스냅샷`적용
  - 사용이유 : 함수를 호출 할 때 이전에 존재했던 값을 유지하기 위함
    - 비동기, 누적카운트에 주로 사용. --> 무분별한 전역변수 남발을 감소시킬 수 있다. 
...
- 포인터(Pointer)
  ...
  - 핵심) `크기가 큰 예)배열인 경우 값 복사시 시스템 부담` --> `포인터 전달`로 해결(슬라이스, 맵 참조 전달)
