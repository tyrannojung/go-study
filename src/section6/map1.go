...
func main() {
  //맵(Map)
  //맵 : 해시테이블, 딕셔너리(파이썬), Key-Value로 자료 지정
  //레퍼런스 타입(참조 값 전달)
  ...
  //예제1
  var map1 map[string] int = make(map[string] int) // 정석이다. but 복잡하다
  var map2 = make(map[string]int) //자료형생략
  map3 := make(map[string]int) //리터럴(짧은선언이므로)
